...
parser.add_argument("--num_elems", type=int, default=200)
...
# Base path
base = f"data/P2_ne{ne_val}_Darcy_5000"
...
def compute_loss(u_pred, u_true, loss_type="mse"):
    if loss_type == "mse":
...
def closure(model, batch, loss_type="mse"):
    u_pred = model(batch)
    loss = compute_loss(u_pred, batch.y, loss_type)
    return loss, u_pred
